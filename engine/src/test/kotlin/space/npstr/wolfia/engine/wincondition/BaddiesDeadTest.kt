...
class BaddiesDeadTest : BaseTest() {
    ...
    fun `2 goodies 1 baddie = win condition not met`() {
        ...
        val baddiesDead = BaddiesDead()
        assertFalse { baddiesDead.isMet(game) }
    }

    @Test
    fun `2 goodies = win condition met`() {
        val players: List<Player> = listOf(
                goodie(),
                goodie()
        )
        ...
        val baddiesDead = BaddiesDead()
        assertTrue { baddiesDead.isMet(game) }
    }
